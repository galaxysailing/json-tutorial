...
void lept_copy(lept_value* dst, const lept_value* src) {
    ...
    switch (src->type) {
        ...
        case LEPT_ARRAY:
            ...
            for (size_t i = 0, len = src->u.a.size; i < len; ++i) {
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; ++i) {
                if ((index = lept_find_object_index(rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen)) == LEPT_KEY_NOT_EXIST) {
                    return 0;
                }
...
lept_value* lept_insert_array_element(lept_value* v, size_t index) {
    assert(v != NULL && v->type == LEPT_ARRAY && index <= v->u.a.size && index >= 0);
    /* \todo */
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    assert(v != NULL && v->type == LEPT_ARRAY && index + count <= v->u.a.size && index >= 0 && count >= 0);
    /* \todo */
    for (size_t i = index + count; i <= v->u.a.size; ++i) {
        v->u.a.e[index++] = v->u.a.e[i];
    }
    v->u.a.size -= count;
}
...
void lept_remove_object_value(lept_value* v, size_t index) {
    assert(v != NULL && v->type == LEPT_OBJECT && index < v->u.o.size && index >= 0);
    /* \todo */
    for (int i = index, len = v->u.o.size - 1; i < len; ++i) {
